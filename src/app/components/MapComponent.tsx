          ...
          >
            ...
            {emergencyEvents
              .filter((emergencyEvent) =>
                filterDataByTimestamp(emergencyEvent, timeToShow)
              )
              .filter((emergencyEvent) =>
                hasFilters ? filterDataFunction(emergencyEvent, filters) : true
              )
              .map((emergencyEvent) => (
                <Marker
                ...
                >
                  <Popup>
                    {isDebug ? (
                      ...
                      <EmergencyEventInfo emergencyEvent={emergencyEvent} />
                    )}
                  </Popup>
                </Marker>
              ))}
...
const EmergencyEventInfo = ({
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <div
        dangerouslySetInnerHTML={{ __html: emergencyEvent.description ?? "" }}
      />
    </div>
  );
};
