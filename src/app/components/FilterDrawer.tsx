      ...
      <ScrollArea className="overflow-y-auto">
        <div className="px-4 flex flex-col gap-4">
          ...
          {Object.entries(facets).map(
            ([facetCategory, value]: [string, Record<string, number>]) => (
              <StatisticBox
                key={facetCategory}
                title={fiFI[facetCategory] || facetCategory}
              >
                <div className="flex flex-col gap-2 relative">
                  {Object.entries(value).map(([facetTitle, facetValue]) => (
                    <div key={facetTitle} className="flex justify-between">
                      <span className="text-sm text-muted-foreground capitalize">
                        {fiFI[facetTitle] || facetTitle}
                      </span>
                      <div className="flex gap-3 items-center">
                        <span className="text-xs  ">{facetValue}</span>
                        <Checkbox
                          checked={filters[
                            facetCategory as keyof Filters
                          ].includes(facetTitle)}
                          onCheckedChange={(checked) =>
                            onChecked(facetCategory, facetTitle, checked)
                          }
                          id={facetTitle}
                          className="size-6"
                        />
